...
function renderPage()
{
    ...
    if (isset($_SERVER["QUERY_STRING"]) && startswith($_SERVER["QUERY_STRING"],'do=picwall'))
    {
        // Optionnaly filter the results:
        $links=array();
 		if (!empty($_GET['searchtermpic'])) $links = $LINKSDB->filterFulltext($_GET['searchtermpic']);
        elseif (!empty($_GET['searchtagspic']))   $links = $LINKSDB->filterTags(trim($_GET['searchtagspic']));
        else {
    ...
    // -------- Picture wall recherche	
	    if (isset($_SERVER["QUERY_STRING"]) && startswith($_SERVER["QUERY_STRING"],'searchtermpic'))
    {
        // Optionnaly filter the results:
        $links=array();
        if (!empty($_GET['searchtermpic'])) $links = $LINKSDB->filterFulltext($_GET['searchtermpic']);
    ...
    // -------- Picture wall tag
	    if (isset($_SERVER["QUERY_STRING"]) && startswith($_SERVER["QUERY_STRING"],'searchtagspic'))
    {
        // Optionnaly filter the results:
        $links=array();
        if (!empty($_GET['searchtagspic']))  {
		$tagpic=array();
		$tagpic=explode(' ',trim($_GET['searchtagspic']));
		$tags= $tagpic;
        ...
        {
            $taglistpic[$value] = $value;
        }
		$links = $LINKSDB->filterTags(trim($_GET['searchtagspic']));
        } else $links = $LINKSDB;
        $body='';
        $linksToDisplay=array();
    ...
    // -------- User clicks on a tag in result count in piacwall : Remove the tag from the list of searched tags in picwall(searchtags=...)
    if (isset($_GET['removetagpic']))
    {
        // Get previous URL (http_referer) and remove the tag from the searchtags parameters in query.
        if (empty($_SERVER['HTTP_REFERER'])) { header('Location: ?'); exit; } // In case browser does not send HTTP_REFERER
        parse_str(parse_url($_SERVER['HTTP_REFERER'],PHP_URL_QUERY), $params);
        if (isset($params['searchtagspic']))
        {
            $tags = explode(' ',$params['searchtagspic']);
            $tags=array_diff($tags, array($_GET['removetagpic'])); // Remove value from array $tags.
            if (count($tags)==0) unset($params['searchtagspic']); else $params['searchtagspic'] = implode(' ',$tags);
            unset($params['page']); // We also remove page (keeping the same page has no sense, since the results are different)
        }
		if (count($tags)==0) header('Location: ?do=picwall'); else header('Location: ?'.http_build_query($params));
        exit;
    }	

    // -------- User clicks on a search in result : Remove the search  (searchtags=...)
    if (isset($_GET['removesearch']))
    {
        // Get previous URL (http_referer) and remove the tag from the searchtags parameters in query.
        if (empty($_SERVER['HTTP_REFERER'])) { header('Location: ?'); exit; } // In case browser does not send HTTP_REFERER
        parse_str(parse_url($_SERVER['HTTP_REFERER'],PHP_URL_QUERY), $params);
	...
	// -------- User clicks on a search in result in picwall : Remove the search in picwall (searchtags=...)
    if (isset($_GET['removesearchpic']))
    {
        // Get previous URL (http_referer) and remove the tag from the searchtags parameters in query.
        if (empty($_SERVER['HTTP_REFERER'])) { header('Location: ?'); exit; } // In case browser does not send HTTP_REFERER
        parse_str(parse_url($_SERVER['HTTP_REFERER'],PHP_URL_QUERY), $params);
		header('Location: ?do=picwall');  
        exit;
    }	
